...
getLatestFace().then(function (addr, err) {
    if(err){
        ...
        curr_face.methods.getEncoding().call({
            gas: 4500000000
        }).then(function(data){
            convertEncodingToDecimal(data).then(console.log);
        });
    }
});

function convertEncodingToDecimal(encoding){
    return new Promise(function(resolve, reject){
        try{
            for(var i in encoding){
                encoding[i] = encoding[i] * .000000000000001;
            }
            resolve(encoding);
        } catch(err){
            reject(err);
